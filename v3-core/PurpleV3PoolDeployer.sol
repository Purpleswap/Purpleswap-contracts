...
import './PurpleV3Pool.sol';

interface Sfs {
   function assign(uint256 _tokenId) external returns (uint256);
}

contract PurpleV3PoolDeployer is IPurpleV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IPurpleV3PoolDeployer
    ...
    function deploy(
        ...
        pool = address(new PurpleV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
