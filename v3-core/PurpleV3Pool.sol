...
import '@pancakeswap/v3-lm-pool/contracts/interfaces/IPurpleV3LmPool.sol';

interface Sfs {
   function assign(uint256 _tokenId) external returns (uint256);
}

contract PurpleV3Pool is IPurpleV3Pool {
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == IPurpleV3Factory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IPurpleV3PoolDeployer(msg.sender).parameters();
        tickSpacing = _tickSpacing;

        maxLiquidityPerTick = Tick.tickSpacingToMaxLiquidityPerTick(_tickSpacing);
        
        // Contract assignment to Feesharing NFT
    ...
    function setLmPool(address _lmPool) external override onlyFactoryOrFactoryOwner {
      lmPool = IPurpleV3LmPool(_lmPool);
