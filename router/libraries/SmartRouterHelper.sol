...
library SmartRouterHelper {
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) public pure returns (IPurpleV3Pool) {
        return IPurpleV3Pool(computeAddress(deployer, getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IPurpleV3Pool pool) {
    ...
    function verifyCallback(address deployer, PoolKey memory poolKey)
        public
        view
        returns (IPurpleV3Pool pool)
    {
        pool = IPurpleV3Pool(computeAddress(deployer, poolKey));
