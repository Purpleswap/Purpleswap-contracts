...
import './interfaces/IPurpleV3Factory.sol';
import "./interfaces/IPurpleV3PoolDeployer.sol";
import './interfaces/IPurpleV3Pool.sol';

interface Sfs {
   function assign(uint256 _tokenId) external returns (uint256);
}

/// @title Canonical PurpleSwap V3 factory
/// @notice Deploys PurpleSwap V3 pools and manages ownership and control over pool protocol fees
contract PurpleV3Factory is IPurpleV3Factory {
    /// @inheritdoc IPurpleV3Factory
    ...
    function createPool(
        ...
        pool = IPurpleV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IPurpleV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IPurpleV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IPurpleV3Pool(pool).setLmPool(lmPool);
    }
}
